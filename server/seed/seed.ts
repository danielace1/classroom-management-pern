...
const seed = async () => {
  ...
  if (data.subjects.length) {
    ...
    await db
      ...
      .onConflictDoNothing({ target: subjects.courseCode });
  }

  const subjectCodes = data.subjects.map((subject) => subject.courseCode);
  const subjectRows =
    subjectCodes.length === 0
      ? []
      : await db
          .select({ id: subjects.id, courseCode: subjects.courseCode })
          .from(subjects)
          .where(inArray(subjects.courseCode, subjectCodes));
  const subjectMap = new Map(
    subjectRows.map((row) => [row.courseCode, row.id]),
  );
